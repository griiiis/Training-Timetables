...
public class GameTypeControllerTest : IClassFixture<CustomWebApplicationFactory<Program>>
{
    private readonly HttpClient _client;
    private readonly CustomWebApplicationFactory<Program> _factory;

    public GameTypeControllerTest(CustomWebApplicationFactory<Program> factory)
    {
        _factory = factory;
    ...
    [Fact]
    public async Task IndexGameTypesAsContestAdmin()
    {
        //login, get jwt
        var response =
    ...
    [Fact]
    public async Task IndexGameTypesAsRandomUser()
    {
        var msg = new HttpRequestMessage(HttpMethod.Get, "/api/v1.0/GameTypes");
        msg.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
    ...
    [Fact]
    public async Task CreateGameType()
    {
        //login, get jwt
        var response =
            await _client.PostAsJsonAsync("api/v1.0/identity/Account/Login", new {Email="admin@eesti.ee", Password="Mina!1"});
        ...
        var gameType = new App.DTO.v1_0.GameType
        {
            GameTypeName = "Tennis",
        };
        
        var json = JsonConvert.SerializeObject(gameType);
        var content = new StringContent(json, Encoding.UTF8, "application/json");
        var msg = new HttpRequestMessage(HttpMethod.Post, "/api/v1.0/GameTypes")
        {
            Content = content
        };
        msg.Headers.Authorization = new AuthenticationHeaderValue("Bearer", loginData.Jwt);
