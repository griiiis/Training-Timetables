...
public class ContestTypeControllerTest : IClassFixture<CustomWebApplicationFactory<Program>>
{
    private readonly HttpClient _client;
    private readonly CustomWebApplicationFactory<Program> _factory;

    public ContestTypeControllerTest(CustomWebApplicationFactory<Program> factory)
    {
        _factory = factory;
    ...
    [Fact]
    public async Task IndexContestTypesAsContestAdmin()
    {
        //login, get jwt
        var response =
    ...
    [Fact]
    public async Task IndexContestTypesAsRandomUser()
    {
        var msg = new HttpRequestMessage(HttpMethod.Get, "/api/v1.0/ContestTypes");
        msg.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
    ...
    [Fact]
    public async Task CreateContestType()
    {
        //login, get jwt
        var response =
            await _client.PostAsJsonAsync("api/v1.0/identity/Account/Login", new {Email="admin@eesti.ee", Password="Mina!1"});
        ...
        var contestType = new App.DTO.v1_0.ContestType
        {
            ContestTypeName = "New ContestType",
            Description = "Something"
        };
        
        var json = JsonConvert.SerializeObject(contestType);
        var content = new StringContent(json, Encoding.UTF8, "application/json");
        var msg = new HttpRequestMessage(HttpMethod.Post, "/api/v1.0/ContestTypes")
        {
            Content = content
        };
        msg.Headers.Authorization = new AuthenticationHeaderValue("Bearer", loginData!.Jwt);
