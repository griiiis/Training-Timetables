    ...
    where TDbContext : DbContext
{
    ...
    {
        return CreateQuery(userId, noTracking).ToList().Select(de => Mapper.Map(de)).AsEnumerable();
    }
    ...
    public async virtual Task<IEnumerable<TDalEntity>> GetAllAsync(TKey? userId = default, bool noTracking = true)
    {
        return (await CreateQuery(userId, noTracking).ToListAsync()).Select(de => Mapper.Map(de));
    }

    public async virtual Task<bool> ExistsAsync(TKey id, TKey? userId = default)
    {
        return await CreateQuery(userId).AnyAsync(e => e.Id.Equals(id));
    }

    public async virtual Task<int> RemoveAsync(TDalEntity entity, TKey? userId = default)
    {
        if (userId == null || userId.Equals(default))
        {
    ...
    public async virtual Task<int> RemoveAsync(TKey id, TKey? userId = default)
    {
        if (userId == null || userId.Equals(default))
        {
            return await RepoDbSet
